...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  # codegen. Every error category stays fatal except the EOF+1 range artifact
  # handled at the genhtml step below, so a real problem still fails the build.
  ...
  exec "lcov --capture --directory " & cache & " --base-directory ." &
       " --include \"*/src/UniCompress/*\" --output-file lcov.info --quiet"
  # gcov can attribute a compiler-generated expression (an exported enum's `$`,
  # here) to EOF + 1; `range` is genhtml's documented filter for exactly that
  # artifact. lcov 2.x wants the matching category allowance alongside it.
