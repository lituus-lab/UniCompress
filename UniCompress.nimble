...
# UniCompress — compression codecs for the lituus-lab Uni* family.

version       = "0.1.0"
author        = "lituus-lab"
description   = "Pure-Nim Deflate and zlib codecs (Nim + C-ABI + Python)"
license       = "Apache-2.0"
srcDir        = "src"
...
task docs, "API reference + book into pages/ — what CI publishes":
  rmDir "pages"
  exec "nim doc --index:on --outdir:pages/api --project --hints:off src/UniCompress.nim"
  exec gate("book")
...
task benchmark, "Run Deflate throughput benchmarks":
  exec "nim c -d:release -r --path:src -o:build/bench_deflate benchmarks/bench_deflate.nim"
  done "benchmark"

task oracle, "Run bidirectional differential tests against Python zlib":
  exec gate("pyTest")
  done "oracle"
...
const
  sharedLib =
    when defined(windows): "libUniCompress.dll"
    elif defined(macosx): "libUniCompress.dylib"
    else: "libUniCompress.so"
  staticLib = "libUniCompress.a"  # MinGW `ar` on Windows, so `.a` everywhere.
...
task clibMsvc, "C static library, MSVC ABI (Windows Python extension)":
  ...
  exec "nim c --cc:vcc --app:staticlib --noMain --mm:arc -d:release -d:staticNoAutoInit" &
       " -o:UniCompress.lib src/UniCompress/c_api.nim"
  done "clibMsvc"
...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  exec "nim c --path:src --nimcache:" & cache &
       ...
       " -o:build/test_coverage tests/test_compress.nim"
  exec "./build/test_coverage"
  exec "lcov --capture --directory " & cache & " --base-directory ." &
       " --include \"*/src/UniCompress/*\" --output-file lcov.info --quiet --ignore-errors mismatch"
  # gcov can attribute a compiler-generated expression (an exported enum's `$`,
  # here) to EOF + 1, and that one artefact answers to two names: lcov 2.0, the
  # version ubuntu-latest installs, calls it `unmapped` and rejects `range` as a
  # category outright, while 2.5 calls it `range` and can filter those lines
  # away. Asking which one is there is not optional -- the hardcoded `range`
  # below could only ever have worked on 2.5.
  ...
  rmDir capiCache
  exec "nim c --app:staticlib --noMain --mm:arc -d:release -d:staticNoAutoInit --debugger:native" &
       " --passC:--coverage --passL:--coverage --nimcache:" & capiCache &
       " -o:build/libUniCompress_cov.a src/UniCompress/c_api.nim"
  ...
  exec makeExe & " -C tests/c BIN=test_capi_cov" &
       " LIB=../../build/libUniCompress_cov.a EXTRA_CFLAGS=--coverage"
  ...
  exec "lcov --capture --no-function-coverage --directory " & capiCache &
       " --base-directory ." &
       " --include \"*/src/UniCompress/*\" --output-file build/capi.info --quiet" &
