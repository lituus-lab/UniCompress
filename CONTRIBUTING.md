...
```text
feat: add the zlib stream container
fix(deflate): reject a canonical Huffman tree that is over-subscribed
docs: state the decompression output ceiling
feat(c_api)!: require a non-zero max_output on inflate
```
