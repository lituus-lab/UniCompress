...
"""Build unicompress._core, a Cython extension over the UniCompress C ABI.
...
PKG_DIR = os.path.join(HERE, "unicompress")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniCompress.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniCompress.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniCompress.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniCompress.so", True
...
def nim_project_dir():
    """Where UniCompress.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unicompress from "
...
# The sdist ships the pre-transpiled unicompress/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unicompress", "_core.pyx")
ext = Extension(
    "unicompress._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unicompress", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniCompress"],
)
...
setup(
    ...
    package_data={"unicompress": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unicompress": ["_core.c"]},
    zip_safe=False,
)
